...
void ApplyCheat(const int key, player& spot, vector<zombie>& zombies, vector<pill>& pills)
{
	if (toupper(key) == EAT)//remove all pils from the grid
	{
		...
		{
			it.hidden = !it.hidden; //sets to hidden if not and vice versa
...
void updateSpotCoordinates(const char g[][SIZEX], player& sp, const int key, string& mess, vector<zombie>& zombies, vector<pill>& pills)
{
	...
	case ZOMBIE:
		...
		if (sp.isProtected)
		{
			sp.protectedCount--;
			for (zombie& it : zombies)
				if (it.baseobject.x == sp.baseobject.x && it.baseobject.y == sp.baseobject.y) 
					it.alive = false; 
	...
	case PILL:
		...
		for (pill &pills : pills)
			if (pills.baseobject.x == sp.baseobject.x && pills.baseobject.y == sp.baseobject.y) 
				pills.eaten = true; 
